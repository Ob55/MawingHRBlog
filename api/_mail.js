/* api/_mail.js
 *
 * Zero-dependency SMTP sender (STARTTLS on port 587) shared by the Vercel API
 * functions and the automation scripts. Built for Office 365 but works with any
 * STARTTLS SMTP server. Uses only Node's built-in net/tls — no npm deps.
 *
 * Underscore-prefixed, so Vercel treats it as a helper, not a route.
 *
 * Config (env / GitHub Actions secrets / Vercel env vars):
...
async function sendMail({ to, subject, html }) {
  ...
  const recipient = to;
  if (!recipient) throw new Error('sendMail: no recipient (to) provided.');
...
// ---- "New blog is live" notification to the admin ----------------------------

function buildPublishedEmail(post, siteUrl) {
  const subject = `✅ New blog published: ${post.title}`;
  const img = post.image
    ? `<img src="${escapeHtml(post.image)}" alt="" width="100%" style="max-width:600px;border-radius:10px;display:block;margin:0 0 20px;">`
    : '';
  const visit = siteUrl
    ? `<p style="margin:0 0 22px;"><a href="${escapeHtml(siteUrl)}" style="display:inline-block;background:#1E5BA8;color:#fff;text-decoration:none;font-weight:bold;font-size:15px;padding:12px 26px;border-radius:999px;">Read it on the website →</a></p>`
    : '';
  const html = `
  <div style="font-family:Arial,Helvetica,sans-serif;max-width:600px;margin:0 auto;color:#0B1E3F;">
    <p style="font-size:14px;color:#64748b;margin:0 0 6px;">Mawingu HR — blog</p>
    <h1 style="font-size:20px;margin:0 0 16px;">A new article has just gone live</h1>
    ${img}
    ...
    ${post.author ? `<p style="font-size:14px;color:#64748b;margin:0 0 18px;">By ${escapeHtml(post.author)}</p>` : ''}
    ${visit}
    <p style="font-size:13px;color:#94a3b8;margin:0;line-height:1.6;">
      This article was approved and is now published on the Mawingu HR website. It will appear within about 30 seconds.
    </p>
  </div>`.trim();
  return { subject, html };
}

async function sendPublishedEmail(post, to, siteUrl) {
  const { subject, html } = buildPublishedEmail(post, siteUrl);
